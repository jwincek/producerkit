...
function register_routes(): void {
    ...
    register_rest_route($ns, '/preorders', [
        [
    ...
    register_rest_route($ns, '/preorders/(?P<id>\d+)/status', [
        'methods'             => \WP_REST_Server::CREATABLE,
        'callback'            => __NAMESPACE__ . '\\update_status',
        'permission_callback' => fn () => current_user_can('edit_posts'),
        'args'                => [
            'status' => ['type' => 'string', 'required' => true, 'enum' => Orders\valid_statuses()],
        ],
    ]);
}
...
function index(\WP_REST_Request $request): \WP_REST_Response {
    return new \WP_REST_Response(Orders\get_orders([
