...
final class PaymentMethodsTest extends WP_UnitTestCase {
    ...
    private function make_location(): int {
        return self::factory()->post->create(['post_type' => 'lfuf_location', 'post_status' => 'publish']);
    }

    public function test_sanitizer_drops_hostile_and_unknown_entries(): void {
        $location = $this->make_location();
        update_post_meta($location, '_lfuf_payment_methods', [
            ['type' => 'venmo',   'value' => '@my-farm',            'label' => ''],
            ['type' => 'link',    'value' => 'javascript:alert(1)', 'label' => 'Evil'],
            ['type' => 'link',    'value' => 'https://sq.example/x', 'label' => 'Square'],
            ['type' => 'bitcoin', 'value' => 'bc1xyz',              'label' => ''],
    ...
    public function test_explicit_venmo_entry_wins_over_legacy_handle(): void {
        ...
        update_post_meta($location, '_lfuf_venmo_handle', 'oldhandle');
        update_post_meta($location, '_lfuf_payment_methods', [
            ['type' => 'venmo', 'value' => 'newhandle', 'label' => ''],
        ]);
    ...
    public function test_enrichment_labels_and_badges(): void {
        ...
        update_post_meta($location, '_lfuf_payment_methods', [
            ...
            ['type' => 'cashapp', 'value' => 'myfarm', 'label' => 'Cash App ($myfarm)'],
        ]);
