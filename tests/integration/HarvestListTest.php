...
final class HarvestListTest extends WP_UnitTestCase {
    ...
    public function set_up(): void {
        ...
        $this->kale  = self::factory()->post->create(['post_type' => 'lfuf_product', 'post_status' => 'publish', 'post_title' => 'Kale']);
        update_post_meta($this->kale, '_lfuf_unit', 'bunch');
        $this->bread = self::factory()->post->create(['post_type' => 'lfuf_product', 'post_status' => 'publish', 'post_title' => 'Bread']);
        $this->location = self::factory()->post->create(['post_type' => 'lfuf_location', 'post_status' => 'publish', 'post_title' => 'Stand']);
    ...
    private function seed_orders(): void {
        create_order(['name' => 'A', 'pickup_date' => $this->day1, 'location_id' => $this->location, 'items' => [
            ['product_id' => $this->kale, 'qty' => 3],
            ['product_id' => $this->bread, 'qty' => 1],
        ]]);
        create_order(['name' => 'B', 'pickup_date' => $this->day1, 'location_id' => $this->location, 'items' => [
            ['product_id' => $this->kale, 'qty' => 4],
        ]]);
        create_order(['name' => 'C', 'pickup_date' => $this->day2, 'items' => [
            ['product_id' => $this->bread, 'qty' => 2],
        ]]);
    }
    ...
    public function test_cancelled_orders_are_excluded(): void {
        $this->seed_orders();
        $big = create_order(['name' => 'D', 'pickup_date' => $this->day1, 'location_id' => $this->location, 'items' => [
            ['product_id' => $this->kale, 'qty' => 50],
        ]]);
    ...
    public function test_filters_by_location_and_date_range(): void {
        ...
        $only_day2 = get_harvest_list(['date_from' => $this->day2, 'date_to' => $this->day2]);
        $this->assertCount(1, $only_day2);
        $this->assertSame($this->day2, $only_day2[0]['pickup_date']);

        $this->assertSame([], get_harvest_list(['date_from' => 'garbage', 'date_to' => 'more-garbage']));
    }
