...
final class StructuredDataTest extends WP_UnitTestCase {
    ...
    public function test_product_offer_from_parseable_price_and_availability(): void {
        ...
        upsert(['product_id' => $product, 'status' => 'limited', 'effective_date' => current_time('Y-m-d')]);
    ...
    public function test_location_business_data(): void {
        $location = self::factory()->post->create(['post_type' => 'lfuf_location', 'post_status' => 'publish', 'post_title' => 'Schema Stand']);
        ...
        update_post_meta($location, '_lfuf_ss_schedule', wp_json_encode([['day' => 6, 'open' => '09:00', 'close' => '16:00']]));
        update_post_meta($location, '_lfuf_venmo_handle', 'examplefarm');
        update_post_meta($location, '_lfuf_payment_methods', [['type' => 'snap_ebt', 'value' => '', 'label' => '']]);
